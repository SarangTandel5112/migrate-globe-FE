...
export const addChecklistToCart = async (token: string, checklistId: string) => {
  try {
    ...
    if (!response.ok) {
      throw new Error(result?.error?.message || "Failed to add checklist");
    }
...
export const removeChecklistFromCart = async (token: string, checklistId: string) => {
  try {
    ...
    if (!response.ok) {
      throw new Error(result.error?.message || 'Failed to remove checklist');
    }
...
export const removeInsuranceFromCart = async (token: string, cartInsuranceItemId: string) => {
  try {
    ...
    if (!response.ok) {
      throw new Error(result.error?.message || 'Failed to remove insurance');
    }
