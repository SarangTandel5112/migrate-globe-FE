...
const BlogsCards = ({ blog }: BlogsCardsProps) => {
  ...
  const sortedBlogs = blog.insights_and_updates.sort(
    (a, b) => a.order - b.order
  );
  ...
  return (
    ...
    >
      ...
      >
        ...
        <button onClick={()=>{router.push('insights')}} className="sm:hidden bg-mint-green-50 text-nowrap text-neutrals-700 px-6 py-2 rounded-full text-sm font-medium hover:bg-mint-green-200">
          View All
        </button>
      </motion.div>
      ...
      >
        <button onClick={()=>{router.push('insights')}} className="bg-mint-green-50 text-neutrals-700 px-6 py-2 rounded-full text-sm font-medium hover:bg-mint-green-200">
          View All Blog Posts →
        </button>
      </motion.div>
    </motion.section>
