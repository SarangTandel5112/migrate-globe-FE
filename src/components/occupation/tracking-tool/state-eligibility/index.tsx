...
const StatusIcon = ({
    ...
    return (
        <div
            className={`${commonClasses} ${status === "eligible" ? "border-green-500" : "border-red"} ${classname}`}
        >
            {status === "eligible" ? (
        ...
        <div className="bg-white rounded-xl border border-neutrals-100 p-4 md:p-4">
            ...
            >
                ...
                <svg
                    ...
                    viewBox="0 0 14 8"
                    fill="none"
                    className={`transition-transform ${showStateEligibility ? "rotate-180" : ""}`}
                >
                    <path
                        d="M1 1L7 7L13 1"
                        stroke="#808080"
            ...
            <AnimatePresence initial={false}>
                {showStateEligibility && (
                    ...
                    <div className="flex flex-col md:flex-row md:justify-end gap-2 md:gap-12 mb-6">
                        {[
                            { src: Check, label: "Occupation MAY Be Eligible" },
                            { src: none, label: "Special Conditions Apply" },
                            { src: Frame, label: "Occupation NOT Eligible" },
                        ].map(({ src, label }, idx) => (
                            <div key={idx} className="flex items-center gap-1">
                                <MyImage src={src} alt={label} className="w-4 h-4" height="16" width="16" />
                                <span className="text-neutrals-700 text-xs">{label}</span>
                            </div>
                        ))}
                    </div>

                    <div className="border-t border-neutrals-100 pt-6">
                        {Object.entries(stateEligibility).map(([visaCode, states]: any, index, arr) => {
                                ...
                                <div key={visaCode}>
                                    ...
                                    <div className="hidden md:flex md:flex-row md:items-center gap-6 mb-4 overflow-y-auto">
                                        <div className="bg-mint-green-50 rounded p-5 lg:p-6 min-w-[160px] mb-4 md:mb-0 flex items-center justify-center">
                                            <span className="text-neutrals-700 text-2xl font-semibold">{visaCode}</span>
                                        </div>
                                        <div className="columns-4 gap-2 space-y-2">
                                            {stateEntries.map(([state, status]: any) => (
                                                <div
                                                    key={state}
                                                    className="flex items-center gap-2 px-3 py-1 bg-background-1 border border-neutrals-100 rounded-full whitespace-nowrap "
                                                >
                                                    <StatusIcon status={status} classname="!w-5 !h-5 shrink-0" />
                                                    <span className="text-neutrals-600 text-sm font-normal">{state}</span>
                                                </div>
                                            ))}
                                        </div>
                                    </div>
                                    ...
                                    <div className="md:hidden mb-6">
                                        <div className="flex justify-left mb-3">
                                            ...
                                            <span className="text-neutrals-700 text-xl font-semibold">{visaCode}</span>
                                            </div>
                                        </div>
                                        ...
                                        >
                                            ...
                                            >
                                                <StatusIcon status={status} classname="!w-3 !h-3 shrink-0" />
