...
export default function CheckList({
    ...
    const allFilteredCheckList = useMemo(() => {
        ...
        if (isVisaCategory) {
            return checklists.filter(
                (checklist) =>
                    checklist?.visa_type?.documentId === selectedVisaCategory
            );
        } else {
            return checklists.filter(
    ...
    const handleBookConsltation = () => {
        router.push(
            `/services/zoom-consultation?enquiry=${selectedEnquiryType}&visa=${selectedVisaCategory}`
        );
    };
                    ...
                    <div className="grid grid-cols-12 gap-4">
                        {ENQUIRY_OPTIONS.map((option) => (
                            <ApplicationTypeButton
                                ...
                                onClick={() =>
                                    setSelectedEnquiryType(option.id)
                                }
                                Icon={option.Icon}
                                title={option.title}
